...
import {GuildMember, MessageEmbed, TextChannel, VoiceChannel, VoiceState} from "discord.js";
...
export class music {
  ...
  async playOnJoin(userId: string, url: string, oldState: VoiceState, newState: VoiceState) {
    if (oldState.id === userId && newState.id === userId && oldState.channelId === null && newState.channelId !== null) {
      const queue = this.player.getQueue(newState.guild, newState.guild.publicUpdatesChannel as TextChannel);

      if (!queue.isReady) {
        await queue.join(newState.channel as VoiceChannel);
      }
      queue.play(url)
      console.log("playing")
    }
  ...
  async voiceUpdate(
    ...
    this.playOnJoin("275722227264126987", "https://youtu.be/V6wCmehGCx4", oldState, newState);
    this.playOnJoin("190911244625117185", "morandi beijo la la", oldState, newState);
    this.playOnJoin("319948920413224990", "dj project hotel", oldState, newState);
  ...
  @Slash("p", { description: "Play a song" })
